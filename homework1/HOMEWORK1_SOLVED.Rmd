...
categorical variable would be sex which is either male or female
```{r}
summary(cats)
```
...
#the mean function is used to obtain the average of a set of values. Bwt and Hwt are not recognised as objects on their own, they can only be accessed through cats dataset using the $ sign, they are objects of that dataset

# mean(cats$Bwt, cats$Hwt)
# Error in mean.default(cats$Bwt, cats$Hwt) : 'trim' must be numeric of length one
# this means the mean function can only find the average of one numeric set of values per time
